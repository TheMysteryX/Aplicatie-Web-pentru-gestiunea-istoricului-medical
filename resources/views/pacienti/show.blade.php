...
<div class="top">
    <div class="titlu">
        <h1>Profil pacient: {{ $pacienti->nume }} {{ $pacienti->prenume }}</h1>
    </div>
</div>
...
<div class="middle">
    <div class="card">
            ...
            <p><strong>Data nașterii:</strong> {{ $pacienti->data_nasterii }}</p>
            <p><strong>CNP:</strong> {{ $pacienti->cnp }}</p>
            <p><strong>Telefon:</strong> {{ $pacienti->telefon }}</p>
            <p><strong>Adresă:</strong> {{ $pacienti->adresa }}</p>
            <p><strong>Asigurat:</strong> {{ $pacienti['asigurat/a'] ? 'Da' : 'Nu' }}</p>
    </div>
    <div class="card">
        ...
        <div class="tab-pane fade show active" id="programari" role="tabpanel">
            ...
            <form method="GET" action="{{ route('pacienti.show', $pacienti->id) }}" 
    ...
    <form method="GET" action="{{ route('pacienti.show', $pacienti->id) }}" style="display:flex; flex-direction: row; flex-wrap:wrap; gap:10px; align-items:center;">
...
Data Expirarii:
    ...
    <div>
        ...
        @foreach(range('A','Z') as $lit)
            <a href="{{ route('pacienti.show', array_merge(request()->all(), ['litera'=>$lit, 'pacienti' => $pacienti->id])) }}">
                {{ $lit }}
            </a>
        @endforeach
    </div>
...
<div class ="buttons" style="padding-bottom:30px;">
    <a href="{{ route('pacienti.istoric', $pacienti) }}" class="form-button" style="padding-bottom:30px;">Istoricul Complet al Pacientului</a>
</div>
</div>
